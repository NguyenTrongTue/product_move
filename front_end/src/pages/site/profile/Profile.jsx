import React, { useEffect, useState } from "react";
import ArrowBackIcon from "@mui/icons-material/ArrowBack";
import EditIcon from "@mui/icons-material/Edit";
import DateRangeIcon from "@mui/icons-material/DateRange";
import LocationOnIcon from "@mui/icons-material/LocationOn";
import classNames from "classnames/bind";
import styles from "./Profile.module.scss";
import { Link } from "react-router-dom";
import { useSelector } from "react-redux";
import formatMonth from "~/util/formatMonth";
import httpRequest, { category } from "~/api/httpRequest";
import Modal from "~/components/modal/Modal";
const cx = classNames.bind(styles);

const Profile = () => {
  const currentUser = useSelector((state) => state.user.currentUser);
  const [wordAddress, setWordAddress] = useState();
  const [page, setPage] = useState();
  const [modal, setModal] = useState(false);

  useEffect(() => {
    const getUser = async () => {
      try {
        switch (currentUser.addressCode[0]) {
          case "s":
            var wordAddress = await httpRequest.getAll(category.stores);
            setWordAddress(
              wordAddress.find(
                (item) => item.storeCode === currentUser.addressCode
              )
            );
            setPage("store");
            break;
          case "f":
            var wordAddress1 = await httpRequest.getAll(category.factories);
            setWordAddress(
              wordAddress1.find(
                (item) => item.factoryCode === currentUser.addressCode
              )
            );
            setPage("factory");
            break;
          case "w":
            var wordAddress2 = await httpRequest.getAll(category.warranties);
            setWordAddress(
              wordAddress2.find(
                (item) => item.warrantyCode === currentUser.addressCode
              )
            );
            setPage("warranty");
            break;
          default:
            setPage("admin");
            break;
        }
      } catch (err) {
        console.log(err);
      }
    };
    getUser();
  }, []);

  return (
    <div className={cx("container")}>
      <div className={cx("header")}>
        <Link to={`/${page}/profile`} className={cx("icon")}>
          <ArrowBackIcon />
        </Link>
        <span>My profile</span>
      </div>
      <div className={cx("wrapper")}>
        <div className={cx("right")}>
          <img
            src="data:image/jpeg;base64,"
            alt=""
          />
          <div className={cx("edit")} onClick={() => setModal(true)}>
            <EditIcon />
            <span>Edit Profile</span>
          </div>
        </div>
        <div className={cx("left")}>
          <div className={cx("name")}>
            <span className={cx("fullname")}>{currentUser.fullName}</span>
            <span className={cx("username")}>{currentUser.username}</span>
          </div>
          <div className={cx("desc")}>
            <div className={cx("join")}>
              <DateRangeIcon />
              <span>Joined {formatMonth(currentUser.createdAt)}</span>
            </div>
            <div className={cx("addressCode")}>
              <LocationOnIcon />
              {wordAddress ? (
                <span>{wordAddress.address}</span>
              ) : (
                <span>Bắc Từ Liêm - Hà Nội</span>
              )}
            </div>
          </div>
          <div className={cx("details")}>
            <div className={cx("item")}>
              <div className={cx("key")}>Email Address</div>
              <div className={cx("value")}>{currentUser.email}</div>
            </div>
            <div className={cx("item")}>
              <div className={cx("key")}>Address</div>
              <div className={cx("value")}>{currentUser.address}</div>
            </div>
            <div className={cx("item")}>
              <div className={cx("key")}>Phone No</div>
              <div className={cx("value")}>{currentUser.phone}</div>
            </div>
            <div className={cx("item")}>
              <div className={cx("key")}>Position</div>
              <div className={cx("value")}>
                {currentUser.isAdmin
                  ? "Admin"
                  : currentUser.isManager
                  ? "Manager"
                  : "Staff"}
              </div>
            </div>
          </div>
        </div>
      </div>
      {modal && <Modal setModal={setModal} />}
    </div>
  );
};

export default Profile;
