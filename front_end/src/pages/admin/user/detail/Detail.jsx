import React, { useState, useEffect } from "react";
import ArrowBackIcon from "@mui/icons-material/ArrowBack";
import EditIcon from "@mui/icons-material/Edit";
import DateRangeIcon from "@mui/icons-material/DateRange";
import LocationOnIcon from "@mui/icons-material/LocationOn";
import classNames from "classnames/bind";
import styles from "./Detail.module.scss";
import { Link, useParams } from "react-router-dom";
import httpRequest, { category } from "~/api/httpRequest";
import formatMonth from "~/util/formatMonth";
import ModalEdit from "~/components/modalEdit/ModalEdit";
const cx = classNames.bind(styles);

const Profile = () => {
  const [user, setUser] = useState();
  const [wordAddress, setWordAddress] = useState();
  const [modal, setModal] = useState(false);
  const [type, setType] = useState("");

  const { id } = useParams();
  useEffect(() => {
    const getUser = async () => {
      try {
        const url = `users/${id}`;
        const user = await httpRequest.getStoreWarehouse(url);

        switch (user.addressCode[0]) {
          case "s":
            var wordAddress = await httpRequest.getAll(category.stores);
            setWordAddress(
              wordAddress.find((item) => item.storeCode === user.addressCode)
            );
            setType("Store");
            break;
          case "f":
            var wordAddress1 = await httpRequest.getAll(category.factories);
            setWordAddress(
              wordAddress1.find((item) => item.factoryCode === user.addressCode)
            );
            setType("Factory");
            break;
          case "w":
            var wordAddress2 = await httpRequest.getAll(category.warranties);
            setWordAddress(
              wordAddress2.find(
                (item) => item.warrantyCode === user.addressCode
              )
            );
            setType("Warranty");
            break;
          default:
            setType("");
            break;
        }

        setUser(user);
      } catch (err) {
        console.log(err);
      }
    };
    getUser();
  }, [id]);

  return (
    <div className={cx("container")}>
      <div className={cx("header")}>
        <Link to="/admin/users" className={cx("icon")}>
          <ArrowBackIcon />
        </Link>
        <span>Details</span>
      </div>
      <div className={cx("wrapper")}>
        <div className={cx("right")}>
          <img
            src="data:image/jpeg;base64,"
            alt=""
          />
          <div className={cx("edit")} onClick={() => setModal(true)}>
            <EditIcon />
            <span>Edit Profile</span>
          </div>
        </div>
        {user && (
          <div className={cx("left")}>
            <div className={cx("name")}>
              <span className={cx("fullname")}>{user.fullName}</span>
              <span className={cx("username")}>{user.username}</span>
            </div>
            <div className={cx("desc")}>
              <div className={cx("join")}>
                <DateRangeIcon />
                <span>Joined {formatMonth(user.createdAt)}</span>
              </div>
              <div className={cx("addressCode")}>
                <LocationOnIcon />
                {wordAddress ? (
                  <span>
                    {`${type}: `}
                    {wordAddress.address}
                  </span>
                ) : (
                  <span>{`${type}`} Bắc Từ Liêm - Hà Nội</span>
                )}
              </div>
            </div>
            <div className={cx("details")}>
              <div className={cx("item")}>
                <div className={cx("key")}>Email Address</div>
                <div className={cx("value")}>{user.email}</div>
              </div>
              <div className={cx("item")}>
                <div className={cx("key")}>Address</div>
                <div className={cx("value")}>{user.address}</div>
              </div>
              <div className={cx("item")}>
                <div className={cx("key")}>Phone No</div>
                <div className={cx("value")}>{user.phone}</div>
              </div>
              <div className={cx("item")}>
                <div className={cx("key")}>Position</div>
                <div className={cx("value")}>
                  {user.isAdmin
                    ? "Admin"
                    : user.isManager
                    ? "Manager"
                    : "Staff"}
                </div>
              </div>
            </div>
          </div>
        )}
      </div>
      {modal && <ModalEdit setModal={setModal} user={user} />}
    </div>
  );
};

export default Profile;
